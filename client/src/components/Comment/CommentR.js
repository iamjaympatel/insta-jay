...
const CommentR = ({
  ...
  const handleGetCommentReplies = async () => {
    if (commentReplies.length === comment.commentReplies.length) {
  ...
  const renderToggleRepliesButtonText = () => {
    if (commentReplies.length === comment.commentReplies.length) {
      ...
      return `View replies (${comment.commentReplies.length})`;
    } else if (commentReplies.length < comment.commentReplies.length) {
      return `View replies (${comment.commentReplies.length - commentReplies.length})`;
    }
  };
    ...
    <Fragment>
      <div className={commentClassNames} ref={commentRef}>
        ...
        <div className="comment__content">
          ...
          {dialogDispatch && !caption && comment.commentReplies.length > 0 ? (
            <p
...
export default connect(null, mapDispatchToProps)(CommentR);