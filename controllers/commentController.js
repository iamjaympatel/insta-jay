...
module.exports.createComment = async (req, res, next) => {
  ...
  try {
    ...
    await comment.save();
    await Post.findByIdAndUpdate({_id:postId},{  $push:{'commentData.comments':comment._id}})
    res.status(201).send({
      ...comment.toObject(),
...
module.exports.voteComment = async (req, res, next) => {
  ...
  try {
    ...
    const commentLikeUpdate = await Comment.updateOne(
      {
        _id: commentId,
        'commentVotes.author': { $ne: user._id },
      },
      { $push: { commentVotes: { author: user._id } } },
    );
    ...
    if (!commentLikeUpdate.nModified) {
      ...
      const commentDislikeUpdate = await Comment.updateOne(
        { _id: commentId },
        { $pull: { commentVotes: { author: user._id } } }
      );
...
module.exports.createCommentReply = async (req, res, next) => {
  ...
  try {
    ...
    await Comment.findByIdAndUpdate({_id:parentCommentId},{$push:{commentReplies:commentReply._id}})
    res.status(201).send({
...
module.exports.deleteCommentReply = async (req, res, next) => {
  ...
  try {
    ...
    await Comment.findOneAndUpdate({commentReplies:commentReplyId},{$pull:{commentReplies:commentReplyId}})
...
module.exports.voteCommentReply = async (req, res, next) => {
  ...
  try {
    const commentReplyLikeUpdate = await CommentReply.updateOne(
      {
        _id: commentReplyId,
        'commentReplyVotes.author': { $ne: user._id },
      },
      { $push: { commentReplyVotes: { author: user._id } } }
    );
    // Nothing was modified in the previous query meaning that the user has already liked the comment
    // Remove the user's like
    if (!commentReplyLikeUpdate.nModified) {
      ...
      const commentReplyDislikeUpdate = await CommentReply.updateOne(
        { _id: commentReplyId },
        { $pull: { commentReplyVotes: { author: user._id } } }
      );
...
module.exports.retrieveCommentReplies = async (req, res, next) => {
  ...
  try {
    ...
    const commentReplies = await CommentReply.find({parentComment:parentCommentId}).populate('author','_id username fullName')
